...
| 参数 | 作用 | 用在哪 |
|------|------|--------|
| `-i` | 连同**响应头**一起输出 | 判断是 `200 OK` 还是 `302 Location`（3.1.3 节，区分 SSRF 与开放重定向） |
| `-v` | 输出**请求全过程**（DNS、TCP、请求行、请求头） | 证明本地 curl 从未连接过目标站（3.1.3 节） |
...
### 3.1.1 探针地址是我自己选的：为什么用 `example.com`

**先明确一点**：`http://example.com` **不是题目给的任何线索**。

题目只告诉我们一件事——缺一个 `url` 参数：

```
url parameter is required
```

它没说这个参数该填什么。**填什么是攻击者的自由**——而这恰恰就是 SSRF 的本质。（对比：如果这个值被应用写死成某个固定地址，那就只是一个普通抓取接口，不构成漏洞。）

#### 为什么第一发选 `example.com`

| 准则 | `example.com` 如何满足 |
|------|----------------------|
| **无害** | RFC 2606 / RFC 6761 保留给文档示例使用，无真实业务、不会下线、无 ToS 风险 |
| **内容可辨认** | 固定返回一个只有 `<h1>Example Domain</h1>` 的极简页 → 一眼看出**不是靶机自己的页面** |
| **稳定可预测** | 常年 200，没有 CDN 拦截 / JS 渲染 / 重定向干扰，避免把「探针站抽风」误判成「靶机不支持抓取」 |

核心是**建立对照组**：我需要一个「提前就知道该收到什么内容」的地址。如果一上来就打 `http://127.0.0.1:8000/`，万一返回了内容，我无法区分那是**服务端代取来的**，还是**应用本来就在输出**。用 example.com 做对照，逻辑立刻闭合：

```
我请求的是靶机  ──►  响应体里却是 example.com 的页面
                          ↓
                这份内容只能是【靶机替我去取的】
                          ↓
                    SSRF 成立
...
后续几个请求进一步证明 `url` 的取值是任意的（对应 3.1.5 节的判定要素③）：

| 填入的 `url` | 服务端返回 | 证明了什么 |
|-------------|-----------|-----------|
| `http://example.com` | Example Domain 页面（200，559 字节） | 服务端能**代取外网**内容 ✅ |
| `http://127.0.0.1:8000/api/internal/secret` | `127.0.0.1 is forbidden` | 换成内网地址 → 命中**黑名单逻辑**（服务端确实按我的值解析目标） |
| `http://example.com:8000/api/internal/secret` | `Read timed out` | 换成别的端口 → 服务端**真的去连了**，只是超时 |
| **`http://127.0.0.2:22/`** | **`SSH-2.0-OpenSSH_7.9p1 Debian-10+deb10u2`** | ⭐ **最有力**：服务端连了 22 端口并把 SSH banner 原样带回 → 目标**主机 + 端口完全随攻击者指定** |
| `http://0.0.0.0:3306/` | `Max retries exceeded`（refused） | 换端口 → 连接被拒，说明真的发起了 TCP 连接 |
| `file:///etc/passwd` | `empty hostname` | 无 host 的协议被应用层拒绝（代码强制取 `hostname`） |

`127.0.0.2:22` 那条尤其有说服力：如果服务端只是「对某个固定地址发请求」，绝不可能返回 SSH 的 banner 字符串。

#### 探针地址的选取准则（可复用到其他 SSRF 题）

| 场景 | 推荐探针 | 说明 |
|------|---------|------|
| **有回显 SSRF**（本题） | `example.com`、`httpbin.org`、**自己 VPS 上放一个带唯一标记的文件** | 内容可辨认，能直接坐实「服务端代取」 |
| **盲 SSRF**（无回显） | **自己控制的域名** + DNSLog：`dnslog.cn`、`ceye.io`、Burp Collaborator | 看不到内容，只能靠「靶机来解析我的域名」这个 DNS 请求证明漏洞 |
| **内网探测** | RFC1918 网段逐扫：`192.168.x.x`、`172.16.x.x`、`10.x.x.x` + 逐端口 | 本题提示已给出目标是 `8000` 端口，可直接打 |

**选探针的三条铁律**：

1. **无害** —— 别拿真实业务系统试，避免误伤；
2. **可辨认** —— 返回内容 / 行为能一眼区分「是服务端抓的」还是「应用自己生成的」；
3. **稳定** —— 避免把探针站自己抽风误判成「靶机不支持抓取」。
