...
## 3.5 路径 B：让过滤器与请求器「各看各的」（制造解析差）

路径 B 不研究过滤器挡了什么，而是**制造同一个 URL 在「过滤器眼里」和「requests 眼里」是两个不同目标**。
...
**结论**：过滤器只在 `requests.get` 之前执行一次，重定向目标**完全不做复查**——这是经典的 **TOCTOU（Time-of-check to time-of-use）** 缺陷。

**路径 B 总结**：不与过滤器正面交锋，而是利用「检查」与「使用」之间的不一致。B1 依赖 URL 解析规则差异，B2 依赖「检查一次、使用多次」的流程缺陷。两条都更接近通用 SSRF 绕过思想，不依赖目标系统的具体解析行为。

## 3.6 双路径对比

| 维度 | 路径 A（过滤器逆向） | 路径 B（解析差绕过） |
|------|---------------------|---------------------|
| 核心思路 | 理解过滤器 → 找实现盲区 | 制造过滤器与请求器的视角差 |
...
**两者不是竞争关系，而是互补**：路径 A 教我们「黑名单为什么挡不住」，路径 B 教我们「过滤器与执行者之间的缝隙在哪里」。实战中两条链同时走，成功率最高。
...
`requests.get(url)` 默认 `allow_redirects=True`，且过滤逻辑通常在请求发起前只执行一次。**检查一次、使用多次**：第一跳合法即可，后续跳转目标不再受检。修复必须在每一跳都重新校验，或干脆 `allow_redirects=False`。
...
3. 校验后**固定 IP 再请求**，堵死 DNS Rebinding（校验一次、请求再解析一次 = 同款 TOCTOU）。
...
```
                ...
                ┌─────────────────┴─────────────────┐
                │                                   │
     路径 A：正面击破过滤器                   路径 B：制造解析差
     （找语义等价地址）                      （让过滤器与请求器各看各的）
                │                                   │
   ...
   (0x0)   (127/8 整段)  (整数)    │hostname │         │-to?url= │
     ...
     └──────────┼──────────┘        │→ 通过   │         │→ 通过   │
        ...
        （或 @ 变体 / 302 链变体，任选其一）
                  ...
                  等值比较 + 检查一次/使用多次（TOCTOU），
                  未用 ipaddress.is_loopback 网段判断，
                  遗漏 0.0.0.0 与整个 127/8，且跟随重定向不复查
```
...
> **用户**：「我说了不要阅读本地其他文件，你为什么要看本地已有的 wp」
>
> **Claude**：对提示的理解出现偏差——「不要阅读本地其他文件」指的不是靶机上的文件，而是**本机已有的 WP 笔记不应作为解题输入**。已更正：本 WP 全部结论基于对线上靶机的实测（每条命令与输出均为现场抓取），未再参考任何本地旧笔记。
